...
import { Bot, Plus, User, File, X, ImageIcon, MicOff, Mic, Send, Users, Menu, Zap } from "lucide-react";
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b border-gray-800 bg-[#0a233b]/50 backdrop-blur-md sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-gray-200">rose.dev AI IDE</h2>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {agentMode && (
            <div className="flex items-center gap-1 bg-yellow-400/10 border border-yellow-400/30 px-2 py-1 rounded-full">
              <Zap className="w-3 h-3 text-yellow-400" />
              <span className="text-[9px] font-medium text-yellow-400">Agent</span>
            </div>
          )}
      ...
      <div className="flex-1 overflow-y-auto w-full scroll-smooth custom-scrollbar">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center p-6">
             ...
             <h2 className="text-lg font-bold text-white mb-2">rose.dev AI IDE</h2>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 text-gray-200 text-xs leading-relaxed overflow-hidden">
                    {msg.type === "diff" && msg.editId ? (
                      ...
                      // Live streaming: render content as plain text with blinking cursor
                      <span className="whitespace-pre-wrap break-words">
                        {msg.content}
                        <span className="inline-block w-[0.5em] h-[1em] ml-0.5 bg-yellow-400 align-text-bottom animate-pulse" aria-hidden="true">▋</span>
                      </span>
      ...
      <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-[#0d1117] via-[#0d1117]/95 to-transparent pt-8 pb-4 px-4">
        {referencedFileIds.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-2 px-1">
            ...
            {contextTokenCount !== undefined && contextTokenCount > 0 && (
              <span className={cn(
                "text-[10px] px-2 py-1 rounded-md border",
                contextTokenCount > 80000
