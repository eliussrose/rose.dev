...
interface EditorSectionProps {
  ...
  setSavedContents?: React.Dispatch<React.SetStateAction<Record<string, string>>>;
}
    ...
    <div className="flex-1 flex flex-col bg-[#1e1e1e] border-r border-gray-800 min-w-0">
      ...
      <div className="flex bg-[#181818] border-b border-gray-800 overflow-x-auto custom-scrollbar h-9 flex-shrink-0">
        {openFiles.length === 0 ? (
          ...
          openFiles.map(file => {
            const isUnsaved = file.content !== (savedContents[file.id] ?? file.content);
            return (
              <div
                key={file.id}
                ...
                onAuxClick={(e) => {
                  ...
                  if (e.button === 1) {
                    e.preventDefault();
                    if (isUnsaved && !confirm(`Save changes to ${file.name}?`)) return;
                    closeFile(file.id, e as unknown as React.MouseEvent);
                  }
                }}
                className={cn(
              ...
              >
                ...
                {isUnsaved && (
                  <span className="text-orange-400 text-[14px] leading-none flex-shrink-0" title="Unsaved changes">•</span>
                )}
                <button
                  onClick={(e) => {
                    e.stopPropagation();
            ...
            <Editor
              ...
              onMount={(editor, monaco) => {
                editor.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.KeyS, () => {
                  if (isElectron && saveFileToDisk && activeFile?.diskPath) {
                    saveFileToDisk();
                    // Update savedContents after save
                    if (setSavedContents && activeFile) {
                      setSavedContents(prev => ({ ...prev, [activeFile.id]: activeFile.content || "" }));
                    }
                  } else if (setSavedContents && activeFile) {
                    // In-memory save: mark as saved
                    setSavedContents(prev => ({ ...prev, [activeFile.id]: activeFile.content || "" }));
                  }
                });
